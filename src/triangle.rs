...
impl Renderer {
    fn new(window: &winit::window::Window) -> Self {
        ...
        let (physical_device, queue_family_indices) =
            Renderer::pick_physical_device(&instance, &surface_loader, surface);
        let device =
            Renderer::create_logical_device(&instance, physical_device, &queue_family_indices);
        let (graphics_queue, present_queue) =
            Renderer::get_queue_handles(&device, &queue_family_indices);
        ...
        let swapchain_image_views =
            Renderer::create_image_views(&device, &swapchain_images, swapchain_image_format);
        let depth_image_format = Renderer::find_depth_format(&instance, physical_device);
        let (depth_image, depth_image_memory, depth_image_view) = Renderer::create_depth_resources(
            &instance,
            &device,
            physical_device,
            swapchain_extent,
        ...
        let single_time_command_pool = Renderer::create_command_pool(
            &device,
            vk::CommandPoolCreateFlags::TRANSIENT,
            &queue_family_indices,
        );
        let (uniform_buffers, uniform_buffers_memory, uniform_buffers_mapped) =
            Renderer::create_uniform_buffers(&instance, &device, physical_device);
        let (texture_image, texture_image_memory) = Renderer::create_texture_image(
            &instance,
            &device,
            physical_device,
        ...
        let render_pass =
            Renderer::create_render_pass(&device, swapchain_image_format, depth_image_format);
        let (pipeline, pipeline_layout) = Renderer::create_graphics_pipeline(
            &device,
            swapchain_extent,
            render_pass,
            descriptor_set_layout,
        ...
        let (vertex_buffer_memory, vertex_buffer) = Renderer::create_vertex_buffer(
            &instance,
            &device,
            physical_device,
            single_time_command_pool,
        ...
        let (index_buffer_memory, index_buffer) = Renderer::create_index_buffer(
            &instance,
            &device,
            physical_device,
            single_time_command_pool,
        ...
        let (swap_image_available_semaphores, render_finished_semaphores, in_flight_fences) =
    ...
    fn draw_frame(&mut self, window: &winit::window::Window, window_resized: bool) {
        ...
        unsafe {
            ...
            let result = self.swapchain_loader.acquire_next_image(
                self.swapchain,
                u64::MAX,
                self.swap_image_available_semaphores[self.current_frame],
                vk::Fence::null(),
            );
    ...
    fn create_instance(entry: &ash::Entry) -> ash::Instance {
        ...
        let p_required_validation_layer_names: Vec<*const i8> =
            cstr_required_validation_layer_names
                .iter()
    ...
    fn pick_physical_device(
        ...
        for device in devices {
            let queue_family_indices: QueueFamilyIndices =
                Renderer::find_queue_families(instance, surface_loader, surface, device);

            if Renderer::is_device_suitable(
                &instance,
                surface_loader,
                surface,
                device,
    ...
    fn is_device_suitable(
        ...
        let _properties: vk::PhysicalDeviceProperties =
        ...
        let is_extensions_supported: bool =
            Renderer::check_device_extension_support(instance, device);
        let mut is_swapchain_adequate: bool = false;

        if is_extensions_supported {
            let swapchain: SwapChainSupportDetails =
                Renderer::query_swapchain_support(surface_loader, surface, device);
            is_swapchain_adequate =
                !swapchain.formats.is_empty() && !swapchain.present_modes.is_empty();
        }

        queue_family_indices.graphics.is_some()
            && queue_family_indices.present.is_some()
    ...
    fn create_swapchain(
        ...
        let surface_format: vk::SurfaceFormatKHR =
            Renderer::choose_swap_surface_format(&swapchain.formats);
        let present_mode: vk::PresentModeKHR =
            Renderer::choose_swap_present_mode(&swapchain.present_modes);
        let extent: vk::Extent2D =
            Renderer::choose_swap_extent(width, height, &swapchain.capabilities);
    ...
    fn create_image_views(
        ...
        for image in swapchain_images.iter() {
            swapchain_image_views.push(Renderer::create_image_view(
                device,
                *image,
                swapchain_image_format,
    ...
    fn create_framebuffers(
        ...
        let mut swapchain_framebuffers: Vec<vk::Framebuffer> =
            Vec::with_capacity(swapchain_image_views.len());

        for image_view in swapchain_image_views {
            let attachments: [vk::ImageView; 2] = [*image_view, depth_image_view];
            ...
            swapchain_framebuffers
                .push(unsafe { device.create_framebuffer(&create_info, None).unwrap() });
        }
    ...
    fn create_sync_objects(
        ...
        let mut swap_image_available_semaphores: Vec<vk::Semaphore> =
        ...
        for _ in 0..MAX_FRAMES_IN_FLIGHT {
            unsafe {
                swap_image_available_semaphores.push(
                    device
                        .create_semaphore(&semamphore_create_info, None)
                        .unwrap(),
                );
    ...
    fn recreate_swapchain(&mut self, size: winit::dpi::PhysicalSize<u32>) {
        ...
        self.swapchain_image_views = Renderer::create_image_views(
            &self.device,
            &self.swapchain_images,
            self.swapchain_image_format,
        );
        ...
        self.swapchain_framebuffers = Renderer::create_framebuffers(
            &self.device,
    ...
    fn create_buffer(
        ...
        let buffer: vk::Buffer = unsafe { device.create_buffer(&create_info, None).unwrap() };
        ...
        let allocate_info = vk::MemoryAllocateInfo {
            ...
            memory_type_index: Renderer::find_memory_type(
                instance,
                physical_device,
    ...
    fn create_vertex_buffer(
        ...
        unsafe {
            let data_ptr: *mut Vertex = device
                .map_memory(
    ...
    fn copy_buffer(
        ...
        let command_buffer: vk::CommandBuffer =
            Renderer::begin_single_time_commands(device, command_pool);
    ...
    fn create_descriptor_set_layout(device: &ash::Device) -> vk::DescriptorSetLayout {
        ...
        let bindings: [vk::DescriptorSetLayoutBinding; 2] =
    ...
    fn create_uniform_buffers(
        ...
        let mut uniform_buffers_memory: Vec<vk::DeviceMemory> =
            Vec::with_capacity(MAX_FRAMES_IN_FLIGHT);
        let mut uniform_buffers_mapped: Vec<*mut c_void> = Vec::with_capacity(MAX_FRAMES_IN_FLIGHT);

        for i in 0..MAX_FRAMES_IN_FLIGHT {
            let (buffer_memory, buffer) = Renderer::create_buffer(
                instance,
                device,
                physical_device,
                buffer_size,
    ...
    fn update_uniform_buffers(&self) {
        ...
        unsafe {
            self.uniform_buffers_mapped[self.current_frame].copy_from_nonoverlapping(
    ...
    fn create_descriptor_sets(
        ...
        for i in 0..MAX_FRAMES_IN_FLIGHT {
            ...
            let descriptor_writes: [vk::WriteDescriptorSet; 2] =
                [buffer_descriptor_write, image_descriptor_write];
    ...
    fn create_texture_image(
        ...
        let image = image::ImageReader::open("image2.png")
            .unwrap()
            .decode()
            .unwrap()
            .into_rgba8();
        ...
        let (texture_image, image_memory) = Renderer::create_image(
            instance,
            device,
            physical_device,
            image.width(),
    ...
    fn create_image(
        ...
        let image: vk::Image = unsafe { device.create_image(&create_info, None).unwrap() };
    ...
    fn transition_image_layout(
        ...
        Renderer::end_single_time_commands(
            device,
            single_time_command_pool,
            command_buffer,
            graphics_queue,
    ...
    fn copy_buffer_to_image(
        ...
        let command_buffer: vk::CommandBuffer =
            Renderer::begin_single_time_commands(device, single_time_command_pool);
    ...
    fn create_depth_resources(
        ...
        let (depth_image, depth_image_memory) = Renderer::create_image(
            instance,
            device,
            physical_device,
        ...
        let depth_image_view = Renderer::create_image_view(
            device,
            depth_image,
            depth_format,
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            for i in 0..MAX_FRAMES_IN_FLIGHT as usize {
                self.device
                    .destroy_semaphore(self.swap_image_available_semaphores[i], None);
                self.device
            ...
            self.device
                .destroy_command_pool(self.single_time_command_pool, None);
